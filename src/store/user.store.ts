...
export const useUserStore = create<UserState>()(
  persist(
    ...
    }),
    {
      name: "user-storage",
      partialize: (state) => {
        // เก็บแค่ joinedDate ใน localStorage
        // ข้อมูลอื่นๆ (name, age, gender, icon, score, rank, streak) เป็น realtime ทั้งหมด
        if (!state.user?.profile?.joinedDate) {
          return {};
        }
        ...
        return {
          user: {
            profile: {
              joinedDate: state.user.profile.joinedDate,
              // ไม่เก็บข้อมูลอื่นๆ เพื่อให้เป็น realtime (fetch ใหม่ทุกครั้ง)
            },
          },
        };
      },
